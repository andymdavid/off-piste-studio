...
Use an author URL when it leads to a real identifying page. A plain `Person` with an accurate name is more defensible than a broken or empty profile. When a dedicated profile exists, Google's [ProfilePage structured data guidance](https://developers.google.com/search/docs/appearance/structured-data/profile-page) shows how the page can identify the `Person` it describes through `mainEntity`. The visible biography and article byline still need to agree.
