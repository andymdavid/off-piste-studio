...
Keep the evidence record after the repair. It becomes the regression sample for the next redesign, plugin update or CMS migration. Give each critical template an editorial owner for meaning and a technical owner for output. Use the [schema markup maintenance guide](/insights/schema-markup-change-management-guide) to connect those samples to an authoritative watchlist, change decisions and review triggers. Review stable identifiers and visible facts when the business changes, not whenever a validator changes colour.
