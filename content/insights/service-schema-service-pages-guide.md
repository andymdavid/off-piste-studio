...
If the page says “contact us for a proposal”, don't invent a price. If delivery is available in Perth but the page never says so, don't add Perth for local relevance. Ratings and reviews belong only when the page shows genuine, policy-compliant evidence for the marked-up item. For a service business marking up reviews about itself, use the [review schema eligibility guide](/insights/review-schema-service-business-mistakes-guide) before adding Review or AggregateRating. A complete-looking graph with unsupported fields is less reliable than a smaller truthful graph.
